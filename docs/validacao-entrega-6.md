...
branch `codex/docs-entrega-6`. Verificação de software realizada em 17/09/2026
...
| Hardware inconsistente | Relé/ejeção sem implementação; 3,43 V apresentados como seguros | Manual de interfaces, pinagem e checklist; artefatos antigos marcados como não validados | Agente hardware; ensaio físico futuro | Não inventar circuito aprovado; resultado parcial explícito. |
...
| Pitch/PoC e slides | Rejeição mecânica, métricas e TFLite sem evidência | Removidas afirmações incompatíveis; exemplos rotulados ilustrativos | Agente arquitetura + revisão independente | Busca de termos e revisão dos contratos. |
