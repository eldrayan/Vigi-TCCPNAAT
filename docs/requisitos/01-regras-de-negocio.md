...
| **0.2.0** | Squad Vigi | 08/09/2026 | Separação entre anomalias do produto e falhas técnicas. |
...
| **RN03** | **Sinalização de Não-Conformidades:** Todo recipiente identificado como não-conforme deve ser registrado e sinalizado no dashboard em tempo hábil para intervenção operacional. | Alta | US02, US03 |
| **RN04** | **Autonomia Operacional Offline:** A inspeção, a decisão e a persistência local devem funcionar sem infraestrutura de rede externa, mantendo os eventos para sincronização posterior. | Alta | US05, RNF03, RNF04 |
...
| **RN11** | **Classificação da Não-Conformidade:** Toda não-conformidade física deve possuir exatamente um tipo entre `SEM_TAMPA`, `TAMPA_TORTA` e `AMASSADO`. Falhas técnicas devem usar categoria e código próprios e não podem compor o Pareto de defeitos do produto. | Alta | US01, US03, US04 |
