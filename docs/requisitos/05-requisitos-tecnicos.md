...
| **Protocolo de Comunicação IoT** | **MQTT (Mosquitto + Paho-MQTT via Localhost TCP e Wi-Fi)** | Publicação e consumo desacoplados de telemetria e alarmes; clientes internos conectam via localhost e clientes externos (painéis/supervisórios) via Wi-Fi. | [RNF04](03-requisitos-nao-funcionais.md), [US03](02-requisitos-funcionais.md), [US05](02-requisitos-funcionais.md), [US09](02-requisitos-funcionais.md) |
| **Backend e dados** | **Python 3 + FastAPI + Paho-MQTT + SQLite** | Centraliza na Raspberry Pi o consumo MQTT, a validação, a persistência transacional em SQLite, as consultas analíticas e a entrega de dados por REST/SSE no mesmo ecossistema Python do modelo. | [US02](02-requisitos-funcionais.md), [US03](02-requisitos-funcionais.md), [US04](02-requisitos-funcionais.md), [US07](02-requisitos-funcionais.md), [US08](02-requisitos-funcionais.md), [US10](02-requisitos-funcionais.md), [US11](02-requisitos-funcionais.md), [RNF11](03-requisitos-nao-funcionais.md) |
| **Frontend web** | **JavaScript + React + Chart.js + Lucide + SCSS** | Exibe indicadores, gráficos, histórico e alarmes consumindo exclusivamente a API do backend através do navegador na rede local. | [US02](02-requisitos-funcionais.md), [US03](02-requisitos-funcionais.md), [US04](02-requisitos-funcionais.md), [US07](02-requisitos-funcionais.md), [US08](02-requisitos-funcionais.md), [US10](02-requisitos-funcionais.md), [US11](02-requisitos-funcionais.md), [RNF11](03-requisitos-nao-funcionais.md) |
...
* **Justificativa:** A inferência local na Raspberry Pi 5 elimina o tráfego de imagens em tempo real na rede da fábrica, garantindo decisão instantânea e imunidade total a oscilações ou quedas de internet ([RN04](01-regras-de-negocio.md)). A Raspberry Pi 5 atua como nó central integrado (*Edge Hub*), hospedando localmente o pipeline de visão, a base SQLite, o broker Mosquitto e o backend FastAPI.
...
* **Justificativa:** O **SQLite** é um banco de dados relacional embutido (*serverless*), sem processo de servidor separado e com baixo consumo de memória, armazenamento e processamento. O acesso local no mesmo sistema de arquivos da Raspberry Pi elimina latência de rede e oferece desempenho ideal ao volume do protótipo. Sua conformidade **ACID** e o modo *Write-Ahead Logging* (WAL) preservam a integridade das gravações e permitem que o backend FastAPI consulte métricas e históricos sem bloquear as gravações do laço de inspeção. O campo `sync_status` (`PENDENTE | SINCRONIZADO`) controla a retransmissão ordenada ao broker MQTT ([RNF04](03-requisitos-nao-funcionais.md)).
...
* **Escolha:** O **MQTT** foi escolhido para a comunicação orientada a eventos porque seu padrão *Publish/Subscribe* desacopla quem produz de quem consome os dados. O pipeline publica cada resultado uma única vez no broker Mosquitto local (`localhost`), que o distribui aos clientes inscritos. Assim, o backend FastAPI local, um eventual display físico OLED e outros consumidores na rede podem receber o mesmo evento sem exigir chamadas HTTP bloqueantes ou acoplamento com o modelo.
...
* **Papel do HTTP:** HTTP/SSE continua sendo utilizado entre o backend FastAPI e o frontend React. O FastAPI centraliza os eventos MQTT, aplica as regras de acesso e disponibiliza REST/SSE ao dashboard no navegador. Dessa forma, o navegador não precisa conhecer o broker nem as credenciais MQTT.
...
* **Justificativa:** O **FastAPI** mantém o backend no ecossistema Python já utilizado pelo modelo de visão computacional. Executando localmente na Raspberry Pi, o backend concentra o cliente Paho-MQTT, as regras de validação, o acesso direto ao SQLite e a entrega de dados por REST/SSE. Isso reduz a duplicação de contratos e simplifica a integração entre o modelo e a API.
