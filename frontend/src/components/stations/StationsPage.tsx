...
export function StationsPage({ stations }: PageData) {
  return (
    <PageSection title="Estações" description="Acompanhe o estado e a conectividade dos dispositivos de inspeção.">
            ...
            <article className={styles.card} key={station.id}>
              <header>
                <div>
                  <h3>{station.name}</h3>
                  <p>
                    {station.code} · {station.device_id}
                  </p>
                </div>
                <Status value={online ? "ONLINE" : "OFFLINE"} />
              </header>
              <div className={styles.peripherals}>
                <Peripheral icon={Radio} name="Sensor" status={station.status?.sensor} />
                <Peripheral icon={Camera} name="Câmera" status={station.status?.camera} />
                <Peripheral icon={Cpu} name="Processamento" status={station.status?.processing} />
              </div>
            </article>
          );
        })}
